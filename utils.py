    ...
    while tobe_processed:
        ...
        if not os.path.exists(csv_path):
            ...
            with open(ai_path, 'w') as f:
                writer = csv.writer(f)
                writer.writerow(headers)
            print(f"读取数据包{package_name}中的CSV文件不存在: {csv_path}")
            logger.warning(f"读取数据包{package_name}中的CSV文件不存在: {csv_path}")
            tobe_processed.popleft()
            continue
        ...
        for index, row in df.iterrows():
            try:
                ...
                if image_pattern in seen_patterns:
                    ...
                    modulelist.append(record)  # 可能需要挪位置
        ...
        # 寻找人工复判csv
        while True:
            human_eva_result, info, timestamp, aoi_path = find_matching_csv(package_name)  # 调用你的函数，可能需要传入参数
        ...
        module_is_ng = None
        # print(info["Part_UserOK"])
        # print(type(info["Part_UserOK"]))

        if package_name == timestamp and info["Part_UserOK"] != 0.0:
            # print(info["Part_UserOK"])
            # print(type(info["Part_UserOK"]))
        ...
        elif package_name != timestamp:
            if info["Part_UserOK"] != 0.0:
                for record in modulelist:
                    record.remark = 'AOI_LOSE'
                df.insert(df.columns.get_loc('FLAG') + 1, 'RepairResult', None)
                if results:
                    # 更新原始CSV中的FLAG列
                    for result in results:
                        df.at[result['index'], 'FLAG'] = result['result']
                logger.warning(f"{package_name}人工复判文件缺失")
                print(f"{package_name}人工复判文件缺失")
            else:
                # 确保目标目录存在，如果不存在则创建
                target_dir = "./userok"
                os.makedirs(target_dir, exist_ok=True)
                # print(aoi_path)
                shutil.move(aoi_path, target_dir)
                print(f"{timestamp}是多余的人工复判文件")
                modulelist.clear()
                continue
        ...
        for record in modulelist:
            record.save_to_csv(f"./SMT_20250716-26zzzzzzz_ResultsData.csv")
